...
BloodSync is a **multi-agent AI system** that orchestrates six specialised agents to support emergency transfusion workflows — combining deterministic medical safety rules with Gemini-powered clinical reasoning.

---

## Agent Workflow

BloodSync is architected as a pipeline of six specialised agents, each with a clearly scoped responsibility:

```
1. Intake Agent
   └─ Collects and validates patient demographics, clinical scenario,
      blood group, haemoglobin level, and available donor types

2. Compatibility Agent
   └─ Executes deterministic ABO/Rh compatibility rules
      Flags incompatible donors · Identifies universal O-negative fallback

3. Risk Assessment Agent
   └─ Evaluates haemoglobin threshold · Scores clinical urgency by scenario
      Produces: Low / Moderate / High / Critical risk level

4. Clinical Reasoning Agent  (Gemini 2.0 Flash)
   └─ Receives structured output from agents 1–3
      Generates context-aware escalation recommendation
      Explains compatibility concerns in plain clinical language

5. Handoff Agent
   └─ Produces a structured transfer report for the receiving clinician or lab
      Includes: patient summary · blood type · risk level · donor suitability · next steps

6. Audit Agent
   └─ Persists the full agent session — inputs, outputs, and report — to MongoDB Atlas
      Supports review, audit trail, and future model fine-tuning
```

Each agent is discrete and independently testable. The pipeline is orchestrated by the FastAPI backend and can be extended with Google ADK for stateful multi-turn agent interactions.
...
Google ADK — SequentialAgent Orchestrator
        │
        ├─▶ [1] intake_agent        (LlmAgent + generate_report_id tool)
        ├─▶ [2] compatibility_agent (LlmAgent + check_abo_rh_compatibility — DETERMINISTIC)
        ├─▶ [3] risk_agent          (LlmAgent + assess_transfusion_risk — DETERMINISTIC)
        ├─▶ [4] reasoning_agent     (LlmAgent — Gemini clinical reasoning)
        ├─▶ [5] handoff_agent       (LlmAgent — Gemini report generation)
        └─▶ [6] audit_agent         (LlmAgent + McpToolset → MongoDB MCP Server)
                                                        │
                                                        ▼
                                              MongoDB Atlas
                                            (bloodsync.reports)
...
- **Multi-step workflow orchestration** — coordinates outputs across the six-agent pipeline and synthesises them into a single coherent clinical summary
...
| Technology | Role |
|---|---|
| **Gemini 2.0 Flash** | Clinical Reasoning Agent and Handoff Agent — generates recommendations and structured reports |
| **Vertex AI / AI Studio** | Gemini API endpoint |
| **Cloud Run** | Backend API deployment (containerised FastAPI) |
| **Firebase Hosting** | Static frontend hosting |
| **Google Cloud IAM** | Secure service access |
| **Google ADK 2.2** | `SequentialAgent` + `LlmAgent` — live multi-agent pipeline orchestration |

## Partner Technology

| Technology | Role |
|---|---|
| **MongoDB Atlas** | Audit Agent — persistent storage of all agent sessions, inputs, outputs, and generated reports |
...
```bash
git clone https://github.com/YOUR_ORG/bloodsync-agent.git
cd bloodsync-agent
python -m venv .venv && source .venv/bin/activate
pip install -r backend/requirements.txt
# Node.js required for MongoDB MCP server (npx mongodb-mcp-server)
# Install from https://nodejs.org if not already present
```
...
**Expected output:**
- Risk level: **Critical**
- Compatible: O negative
- Incompatible: A positive, B positive
- Gemini generates: escalation recommendation + structured handoff report
